...
def nav_html(brand: str, current: str) -> str:
    links = [("Deals", "/"), ("Compare", "/compare/"), ("Methodology", "/methodology/")]
    items = []
...
def build_methodology(config: Any, data: dict[str, Any], urls: list[tuple[str, str]]) -> None:
    ...
    """
    schema = {"@context": "https://schema.org", "@type": "WebPage", "name": "VPS Deals methodology", "url": f"{config.domain}/methodology/", "dateModified": updated}
    html = render_page("compare.html", config=config, title=f"Methodology | {config.brand}", description="How vps-deals checks robots.txt, parses official provider pages, rejects ambiguous prices, and publishes source provenance.", canonical_path="/methodology/", content=content, schema=schema, updated_at=updated)
